...
export default function CBTPractice() {
  const [allQuestions, setAllQuestions] = useState<CBTQuestion[]>([]);
  const [course, setCourse] = useState("All");
  const [answers, setAnswers] = useState<Record<string, number>>({});
  const [submitted, setSubmitted] = useState(false);

  const reload = useCallback(() => setAllQuestions(dataStore.getCBT()), []);
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        {questions.map((q, qi) => (
          <div key={q.id} className="bg-card border rounded-xl p-5 card-shadow">
